...
struct bnxt_aux_device {
	const char *name;
	bool (*is_init)(struct bnxt *bp, int idx);
	bool (*is_active)(struct bnxt *bp, int idx);
};

static bool bnxt_auxdev_is_init(struct bnxt *bp, int idx)
{
	return atomic_read(&bp->auxdev_state[idx]) == BNXT_ADEV_STATE_INIT;
}

static bool bnxt_auxdev_is_active(struct bnxt *bp, int idx)
{
	return atomic_read(&bp->auxdev_state[idx]) == BNXT_ADEV_STATE_ADD;
}
...
void bnxt_ulp_stop(struct bnxt *bp)
{
	...
	for (i = 0; i < __BNXT_AUXDEV_MAX; i++) {
		...
		if (!bnxt_aux_devices[i].is_active(bp, i))
			continue;
...
void bnxt_aux_devices_uninit(struct bnxt *bp)
{
	...
	for (idx = 0; idx < __BNXT_AUXDEV_MAX; idx++) {
		if (bnxt_aux_devices[idx].is_init(bp, idx)) {
...
static void bnxt_aux_dev_release(struct device *dev)
{
	...
	atomic_set(&bp->auxdev_state[aux_priv->id], BNXT_ADEV_STATE_NONE);
}
...
void bnxt_aux_devices_del(struct bnxt *bp)
{
	...
	for (idx = 0; idx < __BNXT_AUXDEV_MAX; idx++) {
		if (bnxt_aux_devices[idx].is_active(bp, idx)) {
			auxiliary_device_delete(&bp->aux_priv[idx]->aux_dev);
			atomic_set(&bp->auxdev_state[idx], BNXT_ADEV_STATE_INIT);
		}
	}
}
...
void bnxt_aux_devices_add(struct bnxt *bp)
{
	...
	for (idx = 0; idx < __BNXT_AUXDEV_MAX; idx++) {
		if (bnxt_aux_devices[idx].is_init(bp, idx)) {
			...
			atomic_set(&bp->auxdev_state[idx], BNXT_ADEV_STATE_ADD);
		}
	}
}
...
void bnxt_aux_devices_init(struct bnxt *bp)
{
	...
	for (idx = 0; idx < __BNXT_AUXDEV_MAX; idx++) {
		atomic_set(&bp->auxdev_state[idx], BNXT_ADEV_STATE_NONE);
